...
onMounted(async () => {
  ...
  }, 2000)
  {
    ...
    //   const hnd = playTone(noteFreq[3]["C"])
    //   setTimeout(() => { hnd.stop() }, 500)
    //   console.log('OK')
    // })

    // setTimeout(() => {
    //   const hnd = playTone(noteFreq[3]["C"])
    //   setTimeout(() => { hnd.stop() }, 1000)
    //   console.log('OK')
    // }, 3000)
...
if (typeof window !== 'undefined') {
  window.addEventListener('click', (e) => {
    if (!window.A) {
      ...
      // const synth = new Tone.FMSynth().toDestination();
      const now = Tone.now()
      synth.triggerAttackRelease('C4', '64n', now)
      // synth.triggerAttackRelease('E4', '8n', now + 0.5)
      // synth.triggerAttackRelease('G4', '8n', now + 1)
      console.log('OK', e)
    }
...
<template>
  <!-- <component src="http://unpkg.com/tone" :is="'script'" /> -->
  <!-- <component src="https://cdnjs.cloudflare.com/ajax/libs/tone/15.0.4/Tone.js" :is="'script'" /> -->
  <div class="container">
    <h3>GRAPH</h3>
    <!-- <div>
